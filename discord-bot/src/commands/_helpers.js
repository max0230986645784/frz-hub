...
export function reply(interaction, content, ephemeral = true) { return interaction.reply({ content, ephemeral }); }
...
export function admin(builder) { return builder.setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild); }
export function optionText(option, name, description, required = true) { return option.setName(name).setDescription(description).setRequired(required); }
export function mention(id) { return id ? `<@${id}>` : "Non configuré"; }
