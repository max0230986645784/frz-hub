...
const kinds = {
  ...
  annonce: ChannelType.GuildAnnouncement,
  forum: ChannelType.GuildForum,
};
export const data = admin(
  new SlashCommandBuilder()
    .setName("salon")
    .setDescription("Gérer les salons")
    .addSubcommand((s) =>
      s
        .setName("creer")
        ...
        .addStringOption((o) =>
          o
            ...
            .setDescription("Type")
            .setRequired(true)
            .addChoices(...Object.keys(kinds).map((name) => ({ name, value: name }))),
        )
        .addStringOption((o) => o.setName("nom").setDescription("Nom").setRequired(true))
        .addChannelOption((o) =>
          o.setName("categorie").setDescription("Catégorie").setRequired(false),
        ),
    )
    ...
    .addSubcommand((s) =>
      s
        .setName("renommer")
        .setDescription("Renommer ce salon")
        .addStringOption((o) => o.setName("nom").setDescription("Nom").setRequired(true)),
    )
    ...
    .addSubcommand((s) =>
      s
        .setName("slowmode")
        .setDescription("Configurer le mode lent")
        .addIntegerOption((o) =>
          o
            .setName("secondes")
...
export async function execute(i) {
  ...
  if (sub === "creer") {
    const c = await i.guild.channels.create({
      ...
      type: kinds[i.options.getString("type")],
      parent: i.options.getChannel("categorie")?.id,
    });
    return i.reply({ embeds: [success(`Salon ${c} créé.`)] });
  }
  if (sub === "supprimer") {
  ...
  if (sub === "verrouiller" || sub === "deverrouiller")
    await i.channel.permissionOverwrites.edit(i.guild.roles.everyone, {
      SendMessages: sub === "deverrouiller" ? null : false,
    });
